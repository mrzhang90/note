...
//校验 营业执照
//18位纯数字 or 15位纯数字
//18位的：2位不含IOZSV开头的字母或者数字，纯数字6位，10位不含IOZSV的字母或数字
//18位的：4位不含IOZSV开头的字母或者数字，纯数字6位，8位不含IOZSV的字母或数字
/(^(?:(?![IOZSV])[\dA-Z]){2}\d{6}(?:(?![IOZSV])[\dA-Z]){10}$)|(^(?:(?![IOZSV])[\dA-Z]){4}\d{6}(?:(?![IOZSV])[\dA-Z]){8}$)|(^\d{15}$)/
```
##Date
```js
// 2018/7/6 10:00:00
