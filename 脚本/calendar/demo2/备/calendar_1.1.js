 ...
 * 解决不了的问题是，起始和结束选好后，循环选中的起始和结束的位置有问题
 * 思路是解决clickObj的index值
 */
$(function(){
    //加载日历插件
    calendarLoad();
...
function changeEvent(startTime,callback) {
    ...
    var clickCount=0,clickObj={"0":{},"1":{},"length":0};
    ...
    function crearCalendar(nows,caledCount){
        ...
        for(var n=nows;n<nows+caledCount;n++){
            ...
            nYear=oDate.getFullYear();//获取当前年份
            ...
            //当前月份
            var nDate=new Date();
            toYear=nDate.getFullYear();//获取今天是几年
        ...
        if(clickObj.length==2){
            // console.log(Number(clickObj[0]['index'].split('-')[0]),Number(clickObj[1]['index'].split('-')[0]),clickObj[1]['datadate'])
            selectDay(clickObj[0]['index'],clickObj[1]['index'],clickObj[1]['datadate'])
        }
        //now=nows+caledCount-1;这句在点击下月时，翻两个月
    }
    function selectDay(oIndex,nIndex,date){
        var dY=Number(clickObj[0]['datadate'].split('-')[0]);//获取第一次点击的月份
        var dM=Number(clickObj[0]['datadate'].split('-')[1]);//获取第一次点击的月份
        var hoverY=Number(date.split('-')[0]);
        var hoverM=Number(date.split('-')[1]);
        if(dY==hoverY){
            veri();
        }else if(dY<hoverY){
            if(monthDiff()==1){
        ...
        function veri(){//相差几个月
            if(monthS[0]==dM || monthS[1]==dM){//当前加载的日历月份 == 第一次点击的月份
    ...
    function selectDate(td){
        td.mouseover(function(){
            ...
            if(len==1){
                ...
                oIndex=clickObj[0]['index'];
                selectDay(oIndex,nIndex,date)
            }
        });
        ...
        function elementVal(val){
            if(element.is('input')){
                element.val(val).removeClass('placeholder');
            }
            if(element.is('a')){
                element.text(val);
            }
        ...
        td.click(function(ev){
            ...
            if(clickCount>=2){
                ...
                clickObj={"0":{},"1":{},"length":0}
            }
            clickObj[clickCount]['index']=index;
            clickObj[clickCount]['datadate']=$(this).attr('data-date');
            clickObj['length']=clickCount+1;
            $(this).addClass("active");
            clickCount++;
            var text1=clickObj[0]['datadate'];
            var text2=clickObj[1]['datadate'];
            if(text1==text2){
                clickCount=0;
                clickObj={"0":{},"1":{},"length":0}
