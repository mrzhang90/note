    ...
    function selectDate(td){
        td.mouseover(function(){
            ...
            if(len==1){
                ...
                var nM=Number(clickObj[0]['datadate'].split('-')[1]);//获取第一次点击的月份
                //如果第二次经过的日期，跟第一次点击的日期在同一页，那么从oIndex到nIndexfor循环；
                //反之则从当前页的第一天到nIndex循环
                if(monthS[0]==nM || monthS[1]==nM){//当前加载的日历月份 == 第一次点击的月份
                    for(var i=oIndex;i<=nIndex;i++){
                        $('td').eq(i).addClass("selected-date");
                    }
                }else if(monthS[0]>nM && monthS[1]>nM){
                    for(var i=0;i<=nIndex;i++){
                        $('td').eq(i).addClass("selected-date");
                    }
                }else if(monthS[0]<nM && monthS[1]<nM && toYear<nYear){
                    for(var i=0;i<=nIndex;i++){
                        $('td').eq(i).addClass("selected-date");
                    }
                }
