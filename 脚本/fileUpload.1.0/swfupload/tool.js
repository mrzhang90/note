function fileUpload(config){
        var uploadFile=$('<input type="file" id="uploadFile" name="uploadFile" style="display:none;">');
        var hide=$('<input type="hidden" name="'+config['element'].attr('data-name')+'" value=""/>');
        config['element'].after(uploadFile);
        config['element'].after(hide);
        var browsers=browser();
        if(!Boolean(window.FormData) || (browsers.msie && parseInt(browsers.version)<10)){
            swfUp({
                'id':config['element'].attr('id'),
